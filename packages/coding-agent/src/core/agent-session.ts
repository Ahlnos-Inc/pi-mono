...
import type { BranchSummaryEntry, CompactionEntry, SessionContext, SessionManager } from "./session-manager.ts";
...
export class AgentSession {
	...
	private _isRetryableError(message: AssistantMessage): boolean {
		...
		// Match: overloaded_error, provider returned error, rate limit, 404/408 transient edge misses, 429, 5xx, service unavailable, network/connection errors, WebSocket transport closes/errors, fetch failed, premature stream endings, HTTP/2 closed before response, terminated, retry delay exceeded.
		return /overloaded|provider.?returned.?error|rate.?limit|too many requests|404|408|429|500|502|503|504|service.?unavailable|server.?error|internal.?error|network.?error|connection.?error|connection.?refused|connection.?lost|websocket.?closed|websocket.?error|other side closed|fetch failed|upstream.?connect|reset before headers|socket hang up|ended without|stream ended before message_stop|http2 request did not get a response|timed? out|timeout|terminated|retry delay/i.test(
			err,
		);
	}
